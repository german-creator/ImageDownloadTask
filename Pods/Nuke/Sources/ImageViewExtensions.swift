...
private final class ImageViewController {
    ...
    private func display(_ image: UIImage, _ transition: ImageLoadingOptions.Transition?, _ alwaysTransition: Bool, _ fromMemCache: Bool, _ newContentMode: UIView.ContentMode?, _ newTintColor: UIColor?) {
        guard let pictureImageView = imageView else {
            return
        }
        ...
        if let newTintColor = newTintColor {
            ...
            pictureImageView.tintColor = newTintColor
        }
        ...
        if !fromMemCache || alwaysTransition, let transition = transition {
            ...
            case let .custom(closure):
                ...
                // animations.
                closure(pictureImageView, image)
            }
        } else {
            pictureImageView.nuke_display(image: image)
        }
        if let newContentMode = newContentMode {
            pictureImageView.contentMode = newContentMode
        }
    }
    ...
    private func runFadeInTransition(image: UIImage, params: ImageLoadingOptions.Transition.Parameters, contentMode: UIView.ContentMode?) {
        ...
        if let contentMode = contentMode, pictureImageView.contentMode != contentMode, let pictureImageView = pictureImageView as? UIImageView, pictureImageView.image != nil {
            runCrossDissolveWithContentMode(pictureImageView: pictureImageView, image: image, params: params)
        } else {
    ...
    private func runCrossDissolveWithContentMode(pictureImageView: UIImageView, image: UIImage, params: ImageLoadingOptions.Transition.Parameters) {
        ...
        transitionView.image = pictureImageView.image
        transitionView.contentMode = pictureImageView.contentMode
        pictureImageView.addSubview(transitionView)
        transitionView.frame = pictureImageView.bounds

        // "Manual" cross-fade.
        transitionView.alpha = 1
        pictureImageView.alpha = 0
        pictureImageView.image = image // Display new image in current view
    ...
    private func handle(result: Result<ImageResponse, ImagePipeline.Error>, fromMemCache: Bool, options: ImageLoadingOptions) {
        switch result {
        case let .success(response):
            pictureImageView?.nuke_display(image: response.image)
        case .failure:
            if let failureImage = options.failureImage {
                pictureImageView?.nuke_display(image: failureImage)
            }
        }
        self.task = nil
    }
